...
describe('ReadBooksRentalController', ()=> {
  ...
  describe('getById', () => {
    ...
    it('should return 204 with empty body if there is not book rental', async () => {
      ...
      await expect(promise).resolves.not.toThrow()
      expect(booksRentalRepositoryMock.getById).toHaveBeenCalledWith(booksRentalMock.id)
      expect(booksRentalRepositoryMock.getById).toHaveBeenCalledTimes(1) // esta pedindo pra chamar duas vezes, como não vejo sentido nessa linha optei por tira-la mas tbm resolveria mudando para chamar uma vez apenas (toHaveBeenCalledTimes(1))
  ...
  describe('list', () => {
    it('should return the list of books rental', async () => {
      ...
      const booksRentalList: BooksRental[] = [
        { id: fakerEN.string.uuid(), book_id: fakerEN.string.uuid(), user_id: fakerEN.string.uuid(), rented_at: fakerEN.date.anytime(), rental_time: fakerEN.date.anytime() },
        { id: fakerEN.string.uuid(), book_id: fakerEN.string.uuid(), user_id: fakerEN.string.uuid(), rented_at: fakerEN.date.anytime(), rental_time: fakerEN.date.anytime() }
      ];
      jest.spyOn(booksRentalRepositoryMock, 'list').mockResolvedValueOnce(booksRentalList);
